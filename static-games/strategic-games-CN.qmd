...
多代理决策问题在概念上不同于单代理决策问题。在单代理决策问题中，决策者需要选择一个策略 $s \in \ALPHABET S$，并获得一个收益或**效用** $u(s)$（或者等价地，承担一个成本 $c(s)$）。在多代理决策问题中，代理人收到的效用可能依赖于其他代理人的策略。这种环境被称为**博弈**。我们从最简单的两人博弈环境开始。

假设一个决策问题中有两个玩家。玩家 1（以下简称 $P_1$）选择一个策略 $s_1 \in \ALPHABET S_1$，玩家 2（以下简称 $P_2$）选择一个策略 $s_2 \in \ALPHABET S_2$。当两个玩家都选择了策略后，$P_1$ 获得收益 $u_1(s_1, s_2)$，而 $P_2$ 获得收益 $u_2(s_1, s_2)$。那么玩家应该如何行动呢？
...
两个罪犯因犯罪被捕，但检察官只有证据指控他们犯下较轻的罪行，而不足以指控他们犯下主要罪行。两名罪犯被分别关押，无法交流。检察官同时向两名罪犯提供以下交易：作证指控对方犯下罪行并获释；除非对方也作证指控，这种情况下两人都会因主要罪行被判较轻的刑期（2 年监禁）。如果只有一人作证，作证者将获释，而另一人将因主要罪行被判全刑期（10 年监禁）。如果两人都不接受交易，则两人都因较轻的罪行被判刑（1 年监禁）。

:::

这个示例可以建模为一个两人博弈，具体如下。两名玩家的策略集合为 $\ALPHABET S_1 = \ALPHABET S_2 = \{ \mathsf{A}, \mathsf{R} \}$，其中 $\mathsf{A}$ 表示玩家接受交易，$\mathsf{R}$ 表示玩家拒绝交易。效用函数可以简洁地表示如下，这种表示形式称为博弈的_双矩阵表示_。
